...
export default function AddPatientModal({ isOpen, onClose, onPatientAdded }) {
  ...
  const [entryDate, setEntryDate] = useState(''); // New state for backdating entry
    ...
    try {
      const response = await fetch('http://localhost:5001/api/patients', {
        ...
        body: JSON.stringify({ 
          ...
          dob, 
          gender,
          created_at: entryDate || undefined // Send custom entry date if provided
        }),
      });
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* New Custom Entry Date Field for Historical Data */}
          <div>
